...
@click.option("-t", default=3810,
help="Telomere length threshold (classification)")
@click.pass_context
class train:
	"""Read BAM file(s) and lengths to train model"""
	def __init__(self, ctx, i, threads, lengths, coverage, paired, sample_cols, stela_cols, model, s, t):
		if os.path.isdir(i):
		...
		if self.model_type == "classification":
			mod, all_data, pred_cols, mod_stats = self.train_classification_model(self.all, model, self.pred_cols, paired, self.mod_build, t)
	...
	def read_lengths_data(self, lengths, paired, sample_cols, stela_cols):
		...
		sample_cols = [c.strip() for c in sample_cols.split(',')]
		stela_cols = [c.strip() for c in stela_cols.split(',')]
		print(sample_cols, stela_cols)
	...
	def combine_lengths_reads(self, table, lengths, paired):
		...
		lengths["sample"] = lengths["sample"].astype(str)
		if paired == False:
			merged = pd.DataFrame.merge(table, lengths[["sample", "stela"]], on="sample", how="left")
		if paired == True:
			merged = pd.DataFrame.merge(table, lengths[["sample", "stela_normal", "stela_tumour"]], on="sample", how="left")
		## check data
	...
	def train_classification_model(self, all_data, model_type, pred_cols, paired, mod_build, t):
		mod_dict = {
		...
		all_data = add_short_labels(int(t), all_data)
		all_data = all_data[all_data.columns.drop(list(all_data.filter(regex="Unnamed")))]

		with pd.option_context('display.max_rows', None):
			print(all_data[["sample", "stela", "short"]])

		#if model_type != "stacking":
		#	pred_cols = feature_select(all_data, paired, model_type, pred_type="classification", selected_types= ["recursive", "corr"]) #["recursive", "from_mod"])
		#	print(pred_cols)
		#else:
		...
		def cross_validate_predict(model, cv_method, all_data, pred_cols, paired=False):
			...
			if drop_samples == True:
				dfs = all_data[all_data["short"] == True]
				dfl = all_data[all_data["short"] == False]
				ns = len(dfs.index)
				nl = len(dfl.index)
				print(f"Originally: {ns} short, {nl} long")
				if ns > nl:
					drop_indices = np.random.choice(dfs.index, ns-nl, replace=False)
					print(f"Dropped: {len(drop_indices)} short samples")
				else:
					drop_indices = np.random.choice(dfl.index, nl-ns, replace=False)
					print(f"Dropped: {len(drop_indices)} long samples")
			...
			if paired == False:
				...
				try:
					...
					train["model_pred"] =  np.where(train["long_conf"] <= 0.6, True, False)
				except:
					pass
			if paired == True:
				train[pred] = pd.DataFrame(cross_val_predict(model, train[pred_cols], train[real], cv=cv_dict[cv_method]), columns = pred, index = train.index)
		...
		mod_stats, mod, all_data = cross_validate_predict(mod, "k_fold", all_data, pred_cols, paired)
	...
	def plot_results(self, fig_dir, mod, data, pred_cols, mod_stats, paired, model_type):
		...
		if plot_importance == True:
			...
			forest_importances = pd.Series(importances, index=pred_cols)
			...
			# print(list(tree_importance_sorted_idx))
			...
			# ax1.set_yticklabels(pred_cols[list(tree_importance_sorted_idx)])
		...
		if plot_roc_curve == True:
			...
			ns_auc = sklearn.metrics.roc_auc_score(data["short"], ns_probs)
			md_auc = sklearn.metrics.roc_auc_score(data["short"], md_probs)
			print('No Skill: ROC AUC=%.3f' % (ns_auc))
			print('Model: ROC AUC=%.3f' % (md_auc))
			ns_fpr, ns_tpr, _ = sklearn.metrics.roc_curve(data["short"], ns_probs)
			md_fpr, md_tpr, _ = sklearn.metrics.roc_curve(data["short"], md_probs)
			plt.plot(ns_fpr, ns_tpr, linestyle='--', label='No Skill')
			plt.plot(md_fpr, md_tpr, marker='.', label=f"Model (AUC={str(md_auc)[:4]})")
...
@click.option("-s", default=None,
help="Shortcut to processed bam spreadsheet")
@click.option("--noot", is_flag=True, default=True,
help="Use optimal transport (yes)")
@click.pass_context
class test:
	"""Apply model to sample data for telomere length prediction"""
	def __init__(self, ctx, i, coverage, model, o, s, noot):
		...
		#	  plt.show()
		if self.optimal_transport == True:
			...
			self.xt_table = pd.read_csv(os.path.join(os.path.dirname(__file__), "models", "for_ot.csv"))
			xt = np.array(self.xt_table[["f_0.perc_cov", "r_0.perc_cov", "rest_0.perc_cov"]])
